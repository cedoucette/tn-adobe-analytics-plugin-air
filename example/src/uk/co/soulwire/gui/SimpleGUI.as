...
package uk.co.soulwire.gui
{
	...
	public class SimpleGUI extends EventDispatcher
	{
		...
		private function initContextMenu() : void
		{
			...
			var arr:Array = menu.customItems
			if(arr == null)
			 arr = new Array();
		...
		private function apply(component : Component, extended : Boolean = false) : void
		{
			...
			if (options.hasOwnProperty("target"))
			{
				...
				for (i = 0; i < targets.length; i++)
				{
					...
					{
						try{ target[prop] = component["selected"]; } catch(error:Error){ component["selected"] = target[prop]; } //avoid read-only errors
					}
